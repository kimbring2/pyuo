...
namespace StormLibSharp
{
    public class MpqArchive : IDisposable
    {
        ...
        private MpqArchive(string filePath, MpqArchiveVersion version, MpqFileStreamAttributes listfileAttributes, MpqFileStreamAttributes attributesFileAttributes, int maxFileCount)
        {
            ...
            if (!NativeMethods.SFileCreateArchive(filePath, (uint) 0x00400000 | 0x03000000, 128, out _handle))
        ...
        public MpqFileStream CreateFile(string fileName, uint file_size)
        {
            VerifyHandle();

            IntPtr phFile;
            if (!NativeMethods.SFileCreateFile(_handle, fileName, 0, file_size, 0, 0x00000200, out phFile))
                throw new Win32Exception();

            MpqFileSafeHandle fileHandle = new MpqFileSafeHandle(phFile);
            MpqFileStream fs = new MpqFileStream(fileHandle, _accessType, this);
        ...
        /*
        public void CloseFile(string fileToExtract, string destinationPath)
        {
            VerifyHandle();
